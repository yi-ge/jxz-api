    ...
    manageAddArticles(sys_id,title,content){
        ...
        return Articles.transaction(t=>{
            return SysUsers.findById(sys_id,{
                ...
                user = result.user;
                console.log(user);
                return Articles.insert(Articles.createModel(title,content,user.id,2),{transaction:t});
            }).then(articles=>{
                return Users.update({article_num:user.article_num+1},{
                    where:{id:user.id},
                    transaction:t,
                    lock: t.LOCK.UPDATE,
                }).then(()=>{
                ...
                Articles.formatArticle(articles);
            });
        });
    }
    /**
